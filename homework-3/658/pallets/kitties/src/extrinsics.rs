...
mod dispatches {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn breed(origin: OriginFor<T>, kitty_id_1: u32, kitty_id_2: u32) -> DispatchResult {
            ...
            ensure!(Self::kitty_owner(kitty_id_1).as_ref() == Some(&who), Error::<T>::NotOwner);
            ensure!(Self::kitty_owner(kitty_id_2).as_ref() == Some(&who), Error::<T>::NotOwner);
            let value = Self::breed_kitty(&who, kitty_1.into(), kitty_2.into());
            Self::mint_kitty(&who, value)?;
            Ok(())
        }
        ...
        pub fn sale(
            ...
            let who = ensure_signed(origin)?;
            ensure!(Self::kitty_owner(kitty_id).as_ref() == Some(&who), Error::<T>::NotOwner);
            ensure!(
                !KittiesBid::<T>::contains_key(kitty_id),
                Error::<T>::KittyAlreadyOnSale
            );
            ensure!(until_block > <frame_system::Pallet<T>>::block_number() + T::MinBidBlockSpan::get(),
                Error::<T>::UntilBlockTooSmall);
        ...
        pub fn bid(origin: OriginFor<T>, kitty_id: u32, price: BalanceOf<T>) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ensure!(Self::kitties(kitty_id).is_some(), Error::<T>::InvalidKittyId);
            ensure!(Self::kitties_bid(kitty_id).is_some(), Error::<T>::KittyNotOnSale);
            ensure!(
                 Self::kitty_owner(kitty_id).as_ref() != Some(&who) ,
                Error::<T>::BidForSelf
            );
            ensure!(price >= T::MinBidIncrement::get(), Error::<T>::BidPriceTooLow);
