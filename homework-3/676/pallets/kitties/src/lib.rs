...
pub mod pallet {
    ...
    use super::*;
    use frame_support::traits::{BalanceStatus, Currency, Randomness};
     //  , ReservableCurrency
    ...
    #[derive(Clone, Encode, Decode, PartialEq, RuntimeDebug, TypeInfo, MaxEncodedLen,Serialize, Deserialize)]
    pub struct KittyInfo(pub [u8; 16]);

    // 定义一个别名，用于表示pallet的配置类型`T`的余额类型。
  ...
  */
    ...
    #[pallet::storage]
    pub type KittyInfoList<T: Config> = StorageMap<_, Blake2_128Concat, u32, KittyInfo>;

    //存储一个最大的值
    #[pallet::storage]
    #[pallet::getter(fn max_bind_value)]
    pub type MaxBindValue<T:Config> = StorageMap<_, Blake2_128Concat, u32, u64>;
 
    //存储拍卖结束的最大区块数
    #[pallet::storage]
    #[pallet::getter(fn max_bind_blcok_num)]
    pub type MaxBindBlockNum<T:Config> = StorageMap<_, Blake2_128Concat, u32, BlockNumberFor<T>>;
    ...
    // 每个用户可以有多个KittyID
    #[pallet::storage]
    #[pallet::getter(fn kitty_owner_list)]
