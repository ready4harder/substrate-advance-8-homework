...
mod dispatches {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create(origin: OriginFor<T>,price:u32) -> DispatchResult {
            ...
            //     data:value,
            //  });
           Self::create_with_stake(&who,value,price)?;
            Ok(())
        }
        #[pallet::call_index(1)]
        #[pallet::weight(T::WeightInfo::breed())]
        pub fn breed(origin: OriginFor<T>, kitty_1: u32, kitty_2: u32,breed_price:u32) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            if let (Some(kitty1), Some(kitty2)) = (Kitties::<T>::get(kitty_1), Kitties::<T>::get(kitty_2)) {
                    ...
                    Self::create_with_stake(&who,value,breed_price)?;
                }    
            Ok(())
        }
        #[pallet::call_index(2)]
