...
impl<T: pallet_kitties::Config> frame_support::traits::OnRuntimeUpgrade for ExampleMigration<T> {
    fn on_runtime_upgrade() -> frame_support::weights::Weight {
        ...
        pallet_kitties::NextKittyId::<T>::mutate(|kitty_id| {
            *kitty_id = 0;
        });
    ...
    fn pre_upgrade() -> Result<Vec<u8>, sp_runtime::TryRuntimeError> {
        ...
        log::info!("example pre_upgrade id: {:?}", kitty_id);
        Ok(kitty_id.encode())
    }
    ...
    fn post_upgrade(state: Vec<u8>) -> Result<(), sp_runtime::TryRuntimeError> {
        ...
        log::info!(
            "example post_upgrade before:{:?} after:{:?}",
            kitty_id_after,
            kitty_id_after
        );
        ensure!(kitty_id_after == kitty_id_after, "invalid state");
