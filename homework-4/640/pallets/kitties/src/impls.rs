...
mod impls {
    impl<T: Config> Pallet<T> {
        ...
        fn combine_kitties(who: &T::AccountId, kitty_1: [u8; 16], kitty_2: [u8; 16]) -> [u8; 16] {
            use core::convert::TryInto;
            kitty_1
                .into_iter()
                .zip(kitty_2)
                .zip(Self::generate_random_value(who))
        ...
        fn trade(until_block: BlockNumberFor<T>) -> DispatchResult {
            ...
            for kitty_id in bids {
                ...
                if T::Currency::reserved_balance(&bidder) < price {
                    log::warn!(
                        "Unexpected Kitties bid Currency::reserved_balance less than price at block {:?}, {:?}, {:?}, {}",
                        until_block,
                        owner,
                        bidder,
                        kitty_id
