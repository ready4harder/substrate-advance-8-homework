...
impl<T: pallet_kitties::Config> frame_support::traits::OnRuntimeUpgrade for ExampleMigration<T> {
    fn on_runtime_upgrade() -> frame_support::weights::Weight {
        ...
        log::info!("runtime migration");
        // pallet_kitties::NextKittyId::<T>::mutate(|id| *id = 0);
        Weight::default()
    }
    ...
    fn post_upgrade(state: Vec<u8>) -> Result<(), sp_runtime::TryRuntimeError> {
        let kitty_id_before = u32::decode(&mut &state[..]).map_err(|_| "decode error")?;
        let kitty_id_after = pallet_kitties::NextKittyId::<T>::get();
        ensure!(kitty_id_before == kitty_id_after, "inconsistent state");
        Ok(())
    }
}
