...
async fn e2e_allowances<Client: E2EBackend>(mut client: Client) -> E2EResult<()> {
    ...
    let bob_account = ink_e2e::account_id(ink_e2e::AccountKeyring::Bob);
    let mint = call_builder.mint(bob_account, total_supply);
    let _transfer_res = client
        .call(&ink_e2e::bob(), &mint)
        .submit()
        .await
        .expect("mint failed");
    // when
...
#[ink_e2e::test]
async fn e2e_mint<Client: E2EBackend>(mut client: Client) -> E2EResult<()> {
    // given
    let total_supply = 1_000_000_000;
    let mut constructor = ERC20Ref::new("test".to_owned(), "TEST".to_owned());
    let erc20 = client
    ...
    let total_supply_msg = call_builder.total_supply();
    let total_supply_res = client
        .call(&ink_e2e::alice(), &total_supply_msg)
        .dry_run()
        .await?;

    let balance_of = call_builder.balance_of(alice_account);
    let balance_of_res = client
        .call(&ink_e2e::alice(), &balance_of)
        .dry_run()
        .await?;
    ...
    assert_eq!(total_supply, balance_of_res.return_value(), "balance_of");

    Ok(())
}

#[ink_e2e::test]
async fn e2e_burn<Client: E2EBackend>(mut client: Client) -> E2EResult<()> {
    // given
    let total_supply = 1_000_000_000;
    let mut constructor = ERC20Ref::new("test".to_owned(), "TEST".to_owned());
    let erc20 = client
    ...
    let burn = call_builder.burn(total_supply);
    let _burn_res = client
        .call(&ink_e2e::alice(), &burn)
        .submit()
        .await
        .expect("burn failed");
    ...
    // then
    assert_eq!(0, total_supply_res.return_value(), "total_supply");
    assert_eq!(0, balance_of_res.return_value(), "balance_of");
